...
export default function Shortener() {
    ...
    return (
        <div className="flex flex-col justify-center items-center h-full gap-8">
            ...
            <input id="userInput" className="max-sm:w-4/5 p-4 cursor-pointer rounded-md outline-none ring-2 ring-red-500 focus:ring-white bg-slate-950 hover:bg-slate-900" onChange={(e) => setUserLink(e.target.value)} />
            <button className="ring-2 ring-red-500 rounded-md px-4 py-1 hover:bg-slate-900" onClick={handleShort}>Shorten!</button>
            <div className={`p-2 text-center ${shortingResponse ? 'opacity-100' : 'opacity-0'} ${error ? 'text-red-500' : 'text-green-400'}`}>
